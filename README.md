...
`worker.py` implements a root-only local worker reached through `nexus-worker execute`, which reads a bounded strict-JSON request from stdin, and `nexus-worker guard`, an internal unit `ExecCondition`. The `workload-host` NixOS module (`services.nexus-workload-worker`, disabled by default) generates the wrapper and an immutable JSON config pinning the host identity, an approved Nix-store bundle allowlist, capacity ceilings, and per-slot UID bases and private addresses. The worker validates bundle manifests and sealed definitions with the artifact/catalog modules, re-verifies the real store closure against the manifest (`nix path-info` and `nix-store --verify-path` via the local Nix daemon store), provisions host-escrowed secret bundles for `secretSetRef`-carrying definitions on hosts configured with the all-or-none `secretsProgram`/`secretsConfigFile`/`secretsBundleDir` trio (rejecting such definitions without it as `secret-provisioning-unavailable`), admits dependency-carrying requests only with the controller's `dependenciesResolved` marker, requires the configured storage UUID to be mounted before any data directory is created, journals operations and instances in a root-owned SQLite database, and admits work only within measured capacity minus existing reservations. Capacity accounting is deliberately conservative — live usage may be double-counted alongside reservations — and enforces no storage quota.
...
`console/secrets.py` is the M4 secret-bundle escrow contract (see `console/README.md`): `seal`/`verify`/`provision`/`inspect` turn a private secret directory into an opaque sealed blob plus a strict envelope whose `bundleDigest` is the exact value a recovery manifest's `secretBundle` carries; provisioning writes a fresh root-0700 directory only after envelope schema, manifest binding, blob digest and inner version re-derivation all agree. The cipher primitive is real — `AgeSealer` runs the pinned `age` binary (scheme `age-x25519-v1`, X25519 recipients with ChaCha20-Poly1305 payload AEAD) as a fixed-argv bounded subprocess, feeds the identity through an anonymous memfd rather than a filesystem path, and binds the AAD digest inside the plaintext as a verified header line since age takes no associated data. Key files are verbatim `age-keygen` output: an identity seals and opens, a recipients-only `age1...` file seals without unseal capability — the escrow shape is "seal anywhere with the public recipient, provision only where the identity lives". The dormant `workload-secrets` host module (`services.nexus-workload-secrets`, disabled by default) ships a root-only `nexus-secrets` wrapper pinning `pkgs.age`; `checks.x86_64-linux.workload-secrets` proves the flow in-guest — recipient-only sealing on one VM, verified provisioning on a second VM holding the identity, hash-only content comparison, and fail-closed binding/tamper/foreign-scheme rejection.
